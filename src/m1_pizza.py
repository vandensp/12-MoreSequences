...
def run_test_generate_points_on_circle():
    ...
    expected = [rg.Point(80.0, 100.0),  # because there are 5 evenly spaced points, they are 360/5 degrees away
                rg.Point(59.3, 128.5), #(30 * cos(72),30 * sin(72))
                rg.Point(25.7, 117.6),
                rg.Point(25.7, 82.4),
                rg.Point(59.3, 71.5)]
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range (len(points)):
        point = points[k]
        circle_2 = rg.Circle(point, 10)
        circle_2.fill_color = color
        circle_2.attach_to(window)
        circle_3 = rg.Circle(point, 1)
        circle_3.fill_color = "black"
...
def run_test_pizza():
    ...
    # -------------------------------------------------------------------------
    # Test 4:
    title = 'PIZZA test 4:  7 slices, thick (thickness=10) green lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 3
    pizza(window, circle, 7, 'green', 10)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    # Tests 3 (on the same window):
    title = ('POLYGON test 3:  10 segments with orange lines')
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(100, 100), 80)
    circle.outline_thickness = 3
    polygon(window, circle, 10, 'orange', 5)
    window.continue_on_mouse_click()
...
def run_test_fancy_polygon():
    ...
    title = ('FANCY POLYGON test 4:  99 red lines,'
             + ' hops = 9.')
    window = rg.RoseWindow(1000, 1000, title)

    circle = rg.Circle(rg.Point(500, 500), 500)
    circle.fill_color = 'blue'
    fancy_polygon(window, circle, 99, 48, 'red', 5)
    window.close_on_mouse_click()
